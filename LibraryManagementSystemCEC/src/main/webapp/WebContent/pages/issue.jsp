...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .sidebar-nav li a svg { width: 20px; height: 20px; margin-right: 15px; stroke-width: 2px; }
    ...
    .form-container { background: rgba(20, 10, 30, 0.75); backdrop-filter: blur(10px); padding: 25px; border-radius: 20px; border: 1px solid rgba(188, 19, 254, 0.3); box-shadow: 0 0 15px rgba(188, 19, 254, 0.1); margin-bottom: 25px; }
    #issueForm { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; }
    .form-group { display: flex; flex-direction: column; }
    .form-group label { font-size: 0.9rem; font-weight: 500; color: #aaa; margin-bottom: 8px; }
    .form-group input[type="text"], .form-group input[type="number"], .form-group input[type="date"] { padding: 12px 15px; border: 1px solid #bc13fe; background: rgba(255, 255, 255, 0.05); border-radius: 8px; font-size: 0.95rem; color: #fff; font-family: 'Poppins', sans-serif; transition: all 0.3s ease; }
    .form-group input::placeholder { color: #888; }
    .form-group input:focus { outline: none; border-color: #00e5ff; background: rgba(255, 255, 255, 0.1); box-shadow: 0 0 15px rgba(0, 229, 255, 0.5); }
    .btn { padding: 12px 20px; border: none; border-radius: 8px; font-size: 1rem; font-weight: 600; cursor: pointer; transition: all 0.2s ease; margin-top: 10px; }
    .btn-submit { background: #00e5ff; color: #0d0517; }
    .btn-submit:hover { transform: translateY(-2px); box-shadow: 0 5px 20px rgba(0, 229, 255, 0.4); }
    .btn-cancel { background: #e74c3c; color: white; }
    .btn-cancel:hover { background: #c0392b; transform: translateY(-2px); box-shadow: 0 5px 15px rgba(231, 76, 60, 0.3); }
    .deleteBtn { background: #e74c3c; color: white; padding: 8px 12px; font-size: 0.9rem; font-weight: 500; border: none; border-radius: 6px; cursor: pointer; }
    .deleteBtn:hover { background: #c0392b; transform: translateY(-1px); }
    .search-bar { display: flex; }
    .search-bar input { width: 100%; padding: 12px 15px; border: 1px solid #bc13fe; background: rgba(255, 255, 255, 0.05); border-radius: 8px; font-size: 0.95rem; color: #fff; font-family: 'Poppins', sans-serif; transition: all 0.3s ease; }
    .search-bar input::placeholder { color: #888; }
    .search-bar input:focus { outline: none; border-color: #00e5ff; background: rgba(255, 255, 255, 0.1); box-shadow: 0 0 15px rgba(0, 229, 255, 0.5); }
...
<body>
      ...
      <ul>
        <li>
          <a href="${pageContext.request.contextPath}/dashboard">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"></rect><rect x="14" y="3" width="7" height="7"></rect><rect x="14" y="14" width="7" height="7"></rect><rect x="3" y="14" width="7" height="7"></rect></svg>
            Dashboard
          </a>
        </li>
        <li> 
      ...
      <table id="issuedTable">
        ...
        <tbody>
          ...
          <c:if test="${empty issueList}">
               <tr><td colspan="7" style="text-align: center; color: #aaa;">No books currently issued.</td></tr>
          </c:if>
        </tbody>
      </table>
    </div>
  ...
  <script>
    // Your existing JavaScript for search and starfield background remains the same
